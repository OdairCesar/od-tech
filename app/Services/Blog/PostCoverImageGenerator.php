...
{
    /**
     * Always applied so every cover image matches OD Tec's site identity,
     * regardless of the chosen {@see ImageStyle} or what the LLM wrote.
     */
    ...
    /**
     * Generates a cover image via OpenAI (gpt-image-1, which always returns
    ...
    public function generate(string $imagePrompt, ImageStyle $style): string
    {
        $response = OpenAI::images()->create([
            ...
            'prompt' => implode(' ', [$imagePrompt, $style->promptInstruction(), self::BRAND_DIRECTION]),
            'n' => 1,
